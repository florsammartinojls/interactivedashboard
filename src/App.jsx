...
function fmtEta(s){if(!s)return"";try{const d=new Date(s);if(isNaN(d.getTime()))return s;return d.toLocaleDateString("en-US",{month:"short",day:"numeric"})}catch(e){return s}}

// Seasonal: CV > 0.3, detect peak quarter
function calcSeasonal(coreId,hist){
  ...
  if(mn===0)return null;
  const cv=Math.sqrt(vals.reduce((a,b)=>a+Math.pow(b-mn,2),0)/vals.length)/mn;
  if(cv<=0.3)return null;
  // Find peak quarter
  const qAvg={Q1:0,Q2:0,Q3:0,Q4:0};const qN={Q1:0,Q2:0,Q3:0,Q4:0};
  ...
  const peak=Object.entries(qAvg).sort((a,b)=>b[1]-a[1])[0][0];
  return{cv:cv.toFixed(2),peak};
}
    ...
    <div className="bg-gray-900 border border-gray-700 rounded-xl p-6 w-full max-w-md" onClick={e=>e.stopPropagation()}>
      ...
      <div className="space-y-4">
        <div className="grid grid-cols-2 gap-3">
          ...
          <div><label className="text-sm text-gray-400 block mb-1">Intl Target DOC</label>
            <input type="number" value={l.intlDoc} onChange={e=>setL({...l,intlDoc:+e.target.value})} className="bg-gray-800 border border-gray-600 text-white rounded px-3 py-2 w-full"/></div></div>
        <div><label className="text-sm text-gray-400 block mb-1">Critical</label>
          <select value={l.critMode} onChange={e=>setL({...l,critMode:e.target.value})} className="bg-gray-800 border border-gray-600 text-white rounded px-3 py-2 w-full"><option value="lt">Lead Time</option><option value="custom">Custom</option></select>
        ...
        <div><label className="text-sm text-gray-400 block mb-1">Warning</label>
          <select value={l.warnMode} onChange={e=>setL({...l,warnMode:e.target.value})} className="bg-gray-800 border border-gray-600 text-white rounded px-3 py-2 w-full"><option value="ltbuf">LT+Buffer</option><option value="custom">Custom</option></select>
        ...
        <div className="border-t border-gray-700 pt-4"><label className="text-sm text-gray-400 block mb-3">Filters</label>
...
// === Glossary ===
const DEF_GL=[{term:"C.DSR",desc:"Composite Daily Sales Rate across all bundles."},{term:"DOC",desc:"Days of Coverage = All-In Pcs / DSR."},{term:"All-In Pcs",desc:"Raw+Inbound+PreProc+JFN+ProcQ+JI+FBA."},{term:"Qty Needed",desc:"max(0, Target DOC x DSR - All-In). 90d domestic, 180d intl."},{term:"After DOC",desc:"If buying: (All-In+Qty)/DSR. If not: dash."},{term:"Seasonal",desc:"CV of monthly DSR > 0.3. Shows peak quarter."}];
...
function PurchasingTab({data,stg,onViewCore}){
  ...
  const enriched=useMemo(()=>{
    return(data.cores||[]).filter(c=>{
      ...
      return{...c,status:st,allIn,needQty:nq,needDollar:+(nq*c.cost).toFixed(2),docAfter:calcDocAfter(c,nq),lt,critDays:cd,warnDays:wd,targetDoc:td,vc:v.country||"",seas};
  ...
  return<div className="p-4">
    ...
    {vm==="core"&&<div className="overflow-x-auto rounded-xl border border-gray-800"><table className="w-full"><thead>
      ...
      <td className="py-2 px-2 text-gray-400 text-xs truncate max-w-[100px]" title={c.vc?c.ven+" ("+c.vc+")":c.ven}>{c.ven}</td>
      ...
      <td className="py-2 px-2 text-right text-gray-400 text-xs">{c.moq>0?R(c.moq):"\u2014"}</td>
      <td className="py-2 px-2 text-center">{c.seas&&<span className="text-purple-400 text-xs font-bold" title={c.seas.peak+" Peak, CV:"+c.seas.cv}>{c.seas.peak}</span>}</td>
    ...
    {vm==="vendor"&&vGroups.map(grp=>{const v=grp.v;const noNeed=grp.q===0;const meets=grp.d>=(v.moqDollar||0);const td=getTD(v,stg);
      return<div key={v.name} className="mb-5 border border-gray-800 rounded-xl overflow-hidden">
        ...
        <tbody>{grp.cores.map(c=><tr key={c.id} className="border-t border-gray-800/30 hover:bg-gray-800/20 text-sm">
          <td className="py-2 px-2"><Dot status={c.status}/></td><td className="py-2 px-2 text-blue-400 font-mono text-xs">{c.id}</td><td className="py-2 px-2 text-gray-200 truncate max-w-[200px]">{c.ti}</td><td className="py-2 px-2 text-right">{R(c.dsr)}</td><td className={`py-2 px-2 text-right font-semibold ${dc(c.doc,c.critDays,c.warnDays)}`}>{R(c.doc)}</td><td className="py-2 px-2 text-right">{R(c.allIn)}</td><td className="py-2 px-2 text-right text-gray-400 text-xs">{c.moq>0?R(c.moq):"\u2014"}</td><td className="py-2 px-1 border-l-2 border-gray-600"/><td className="py-2 px-2 text-right">{c.needQty>0?R(c.needQty):"\u2014"}</td><td className="py-2 px-2 text-right text-amber-300">{c.needDollar>0?D(c.needDollar):"\u2014"}</td><td className={`py-2 px-2 text-right ${c.needQty>0?dc(c.docAfter,c.critDays,c.warnDays):"text-gray-500"}`}>{c.needQty>0?R(c.docAfter):"\u2014"}</td><td className="py-2 px-2"><button onClick={()=>onViewCore(c.id)} className="text-blue-400 text-xs px-2 py-1 bg-blue-400/10 rounded">View</button></td>
...
function CoreDetailTab({data,stg,hist,coreId,onBack,onGoBundle}){
  ...
  const coreBundles=useMemo(()=>{
    return coreBundlesAll.map(b=>({...b,fee:feesMap[b.j],sale:salesMap[b.j],pct:totalBDsr>0?+((b.cd/totalBDsr)*100).toFixed(1):0}))
  ...
  if(!core)return<div className="p-4 max-w-4xl mx-auto"><div className="flex items-center gap-3 mb-4"><button onClick={onBack} className="text-gray-400 hover:text-white text-sm">{"\u2190"} Purchasing</button><input type="text" placeholder="Search core ID or title (min 2 chars)..." value={search} onChange={e=>setSearch(e.target.value)} className="bg-gray-800 border border-gray-700 text-white rounded-lg px-4 py-2.5 flex-1 max-w-md text-sm"/></div>{search.length>=2?<div className="space-y-1">{(data.cores||[]).filter(c=>{const q=search.toLowerCase();return c.id.toLowerCase().includes(q)||c.ti.toLowerCase().includes(q)}).slice(0,12).map(c=><button key={c.id} onClick={()=>setSel(c.id)} className="w-full text-left px-4 py-2.5 rounded-lg bg-gray-900/50 hover:bg-gray-800 flex items-center gap-3"><Dot status={getStatus(c.doc,(data.vendors||[]).find(v=>v.name===c.ven)?.lt||30,c.buf,stg)}/><span className="text-blue-400 font-mono text-sm">{c.id}</span><span className="text-gray-300 text-sm truncate">{c.ti}</span><span className="ml-auto text-gray-500 text-xs">DSR:{R(c.dsr)} DOC:{R(c.doc)}</span></button>)}</div>:<p className="text-gray-500 text-sm">Type at least 2 characters.</p>}</div>;
  ...
  return<div className="p-4 max-w-6xl mx-auto">
    ...
    {/* Inbound Shipments */}
    {inboundShipments.length>0&&<div className="bg-gray-900 rounded-xl p-4 mb-4 border border-emerald-800/50">
      <h3 className="text-emerald-400 font-semibold text-sm mb-2">{"\u{1F4E6}"} Inbound Shipments ({inboundShipments.length})</h3>
      <div className="space-y-1">{inboundShipments.map((s,i)=><div key={i} className="flex items-center gap-4 text-sm">
        <span className="text-gray-300">{s.cases} cases</span>
        <span className="text-gray-400">{s.name}</span>
        {s.eta&&<span className="text-emerald-400 font-semibold">ETA: {fmtEta(s.eta)}</span>}
        {s.country&&<span className="text-gray-500 text-xs">{s.country}</span>}
      </div>)}</div></div>}
    {/* Charts */}
    {coreHist.length>0&&<div className="grid grid-cols-1 lg:grid-cols-2 gap-4 mb-4">
      ...
      {bSalesH.length>0&&<div className="bg-gray-900 rounded-xl p-4 border border-gray-800"><h3 className="text-white font-semibold text-sm mb-3">Monthly Sales Units (YoY)</h3><ResponsiveContainer width="100%" height={260}><LineChart data={unitsChart}><CartesianGrid strokeDasharray="3 3" stroke="#374151"/><XAxis dataKey="month" tick={{fill:"#9ca3af",fontSize:11}}/><YAxis tick={{fill:"#9ca3af",fontSize:11}}/><Tooltip {...TT}/><Legend/>{years.map(y=><Line key={"u"+y} dataKey={"u_"+y} stroke={YR_C[y]||"#6b7280"} strokeWidth={2} dot={{r:3}} connectNulls name={""+y}/>)}</LineChart></ResponsiveContainer></div>}
    </div>}
    {/* Pipeline */}
...
function BundleDetailTab({data,stg,hist,bundleId,onBack,onGoCore}){
  ...
  // Chart data with proper month labels
  const chartLabel=h=>{const m=MN[h.m-1]||"";return m+" "+(h.y%100)};
  const revData=useMemo(()=>salesHist.map(h=>({month:chartLabel(h),rev:h.rev,profit:h.profit})),[salesHist]);
  const priceData=useMemo(()=>{const pm={};priceHist.forEach(p=>pm[p.month]=p.avgPrice);return salesHist.map(h=>({month:chartLabel(h),units:h.units,price:pm[h.month]??null}))},[salesHist,priceHist]);
  ...
  return<div className="p-4 max-w-6xl mx-auto">
    ...
    {salesHist.length>0&&<div className="grid grid-cols-1 lg:grid-cols-2 gap-4 mb-4">
      <div className="bg-gray-900 rounded-xl p-4 border border-gray-800"><h3 className="text-white font-semibold text-sm mb-3">Sales History</h3><ResponsiveContainer width="100%" height={240}><BarChart data={salesHist.map(h=>({month:chartLabel(h),units:h.units}))}><CartesianGrid strokeDasharray="3 3" stroke="#374151"/><XAxis dataKey="month" tick={{fill:"#9ca3af",fontSize:9}} angle={-45} textAnchor="end" height={50}/><YAxis tick={{fill:"#9ca3af",fontSize:11}}/><Tooltip {...TT}/><Bar dataKey="units" fill={BLUE} radius={[2,2,0,0]}/></BarChart></ResponsiveContainer></div>
...
export default function App(){
  ...
  useEffect(()=>{load()},[load]);
  // Pass coreInv to purchasing for seasonal calc
