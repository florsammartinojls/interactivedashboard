...
function CoreDetailTab({data,stg,hist,coreId,onBack,onGoBundle}){
  ...
  return<div className="p-4 max-w-6xl mx-auto">
    ...
    <div className="bg-gray-900 rounded-xl p-4 mb-4 border border-gray-800">
      ...
      <p className="text-gray-500 text-xs">{core.ven} {"\u00B7"} {D2(core.cost)} {"\u00B7"} LT:{lt}d {"\u00B7"} Target:{td}d {"\u00B7"} {core.cat}</p>
      <p className="text-gray-600 text-xs mt-1">Inbound data: {(data.inbound||[]).length} total, {inboundShipments.length} matched {inboundShipments.length>0&&<span className="text-emerald-400">{"\u2014"} {inboundShipments.map((s,i)=><span key={i}>{s.core}: {s.cases} cases ETA:{fmtEta(s.eta)} </span>)}</span>}</p></div>
