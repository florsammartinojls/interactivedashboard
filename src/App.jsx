...
let _jid=0;
function jp(url,timeout=60000){return new Promise((resolve,reject)=>{const cb='__jp'+(++_jid)+'_'+Date.now();const timer=setTimeout(()=>{cl();reject(new Error('Timeout'))},timeout);const s=document.createElement('script');function cl(){clearTimeout(timer);delete window[cb];s.parentNode&&s.parentNode.removeChild(s)}window[cb]=d=>{cl();resolve(d)};s.src=url+(url.includes('?')?'&':'?')+'callback='+cb;s.onerror=()=>{cl();reject(new Error('Network error'))};document.head.appendChild(s)})}
function apiCall(action){return jp(API+'?action='+action)}
...
const YR_C={2023:"#8b5cf6",2024:"#3b82f6",2025:"#22c55e"};
...
function calcDocAfter(c,qty){return qty<=0?Math.round(c.doc):c.dsr>0?Math.round((calcAllIn(c)+qty)/c.dsr):999}
function isDomestic(country){return DOMESTIC.includes((country||"").toLowerCase().trim())}
function getTargetDoc(vendor,stg){return isDomestic(vendor?.country)?stg.domesticDoc:stg.intlDoc}
    ...
    <div className="bg-gray-900 border border-gray-700 rounded-xl p-6 w-full max-w-md" onClick={e=>e.stopPropagation()}>
      ...
      <div className="space-y-4">
        <div className="grid grid-cols-2 gap-3">
          <div><label className="text-sm text-gray-400 block mb-1">Domestic Target DOC</label>
            <input type="number" value={l.domesticDoc} onChange={e=>setL({...l,domesticDoc:+e.target.value})} className="bg-gray-800 border border-gray-600 text-white rounded px-3 py-2 w-full"/></div>
          <div><label className="text-sm text-gray-400 block mb-1">International Target DOC</label>
            <input type="number" value={l.intlDoc} onChange={e=>setL({...l,intlDoc:+e.target.value})} className="bg-gray-800 border border-gray-600 text-white rounded px-3 py-2 w-full"/></div>
        </div>
        <div><label className="text-sm text-gray-400 block mb-1">Critical Threshold</label>
          <select value={l.critMode} onChange={e=>setL({...l,critMode:e.target.value})} className="bg-gray-800 border border-gray-600 text-white rounded px-3 py-2 w-full">
            <option value="lt">Lead Time</option><option value="custom">Custom</option></select>
        ...
        <div><label className="text-sm text-gray-400 block mb-1">Warning Threshold</label>
          <select value={l.warnMode} onChange={e=>setL({...l,warnMode:e.target.value})} className="bg-gray-800 border border-gray-600 text-white rounded px-3 py-2 w-full">
            <option value="ltbuf">LT+Buffer</option><option value="custom">Custom</option></select>
        ...
        <div className="border-t border-gray-700 pt-4"><label className="text-sm text-gray-400 block mb-3">Data Filters</label>
          <div className="space-y-3">
            <div className="flex items-center justify-between"><span className="text-sm text-gray-300">Active</span>
              <select value={l.fA} onChange={e=>setL({...l,fA:e.target.value})} className="bg-gray-800 border border-gray-600 text-white rounded px-2 py-1 text-sm w-24">
                <option value="yes">Yes</option><option value="no">No</option><option value="all">All</option></select></div>
            <div className="flex items-center justify-between"><span className="text-sm text-gray-300">Visible</span>
              <select value={l.fV} onChange={e=>setL({...l,fV:e.target.value})} className="bg-gray-800 border border-gray-600 text-white rounded px-2 py-1 text-sm w-24">
                <option value="yes">Yes</option><option value="no">No</option><option value="all">All</option></select></div>
            <div className="flex items-center justify-between"><span className="text-sm text-gray-300">Ignored</span>
              <select value={l.fI} onChange={e=>setL({...l,fI:e.target.value})} className="bg-gray-800 border border-gray-600 text-white rounded px-2 py-1 text-sm w-24">
                <option value="blank">Blank (not ignored)</option><option value="set">Not Blank (ignored)</option><option value="all">All</option></select></div>
          </div></div>
      </div>
      <div className="flex gap-3 mt-6"><button onClick={()=>{setS(l);onClose()}} className="flex-1 bg-blue-600 text-white rounded-lg py-2 font-medium">Save</button>
        <button onClick={onClose} className="flex-1 bg-gray-700 text-white rounded-lg py-2 font-medium">Cancel</button></div>
    </div></div>}

const DEF_GL=[{term:"C.DSR",desc:"Composite Daily Sales Rate across all bundles."},{term:"DOC",desc:"Days of Coverage = All-In Pcs / DSR."},{term:"All-In Pcs",desc:"Raw+Inbound+PreProc+JFN+ProcQ+JI+FBA."},{term:"Qty Needed",desc:"max(0, Target DOC x DSR - All-In). Target = 90d domestic, 180d international."},{term:"After DOC",desc:"If buying: (All-In+Qty)/DSR. If not buying: shows dash."}];
...
function PurchasingTab({data,stg,onViewCore}){
  ...
  const enriched=useMemo(()=>{
    return(data.cores||[]).filter(c=>{
      ...
      if(stg.fI==="blank"&&!!c.ignoreUntil)return false;if(stg.fI==="set"&&!c.ignoreUntil)return false;
      return true;
    }).map(c=>{
      const v=vMap[c.ven]||{};const lt=v.lt||30;const td=getTargetDoc(v,stg);
      ...
      const allIn=calcAllIn(c);const nq=calcNeedQty(c,td);
      return{...c,status:st,allIn,needQty:nq,needDollar:+(nq*c.cost).toFixed(2),docAfter:calcDocAfter(c,nq),lt,critDays:cd,warnDays:wd,targetDoc:td,vendorCountry:v.country||""};
    }).filter(c=>{
      if(vf&&c.ven!==vf)return false;if(sf&&c.status!==sf)return false;
      if(minDoc>0&&c.doc<minDoc)return false;
      if(nf==="need"&&c.needQty<=0)return false;if(nf==="ok"&&c.needQty>0)return false;
      return true;
  ...
  return<div className="p-4">
    <div className="flex flex-wrap gap-2 items-center mb-4">
      ...
      <select value={vf} onChange={e=>setVf(e.target.value)} className="bg-gray-800 border border-gray-700 text-gray-300 text-sm rounded-lg px-2 py-1.5"><option value="">All Vendors</option>{(data.vendors||[]).map(v=><option key={v.name} value={v.name}>{v.name}</option>)}</select>
      ...
      <div className="flex gap-2 ml-auto text-xs"><span className="flex items-center gap-1"><span className="w-2 h-2 rounded-full bg-red-500"/>{sc.critical}</span><span className="flex items-center gap-1"><span className="w-2 h-2 rounded-full bg-amber-500"/>{sc.warning}</span><span className="flex items-center gap-1"><span className="w-2 h-2 rounded-full bg-emerald-500"/>{sc.healthy}</span></div>
    </div>
    {vm==="core"&&<div className="overflow-x-auto rounded-xl border border-gray-800"><table className="w-full"><thead>
      <tr className="bg-gray-900/80 text-xs text-gray-400 uppercase">
        <th className="py-3 px-2 w-8"/><th className="py-3 px-2 text-left">Core</th><th className="py-3 px-2 text-left">Vendor</th><th className="py-3 px-2 text-left">Title</th>
        <th className="py-3 px-2 text-right">DSR</th><th className="py-3 px-2 text-right">7D</th><th className="py-3 px-2 text-center">T</th><th className="py-3 px-2 text-right">DOC</th>
        <th className="py-3 px-2 text-right">All-In<InfoTip text="Raw+Inb+PP+JFN+PQ+JI+FBA"/></th>
        <th className="py-3 px-1 border-l-2 border-gray-600"/>
        <th className="py-3 px-2 text-right text-blue-400" colSpan={3}>Purchase Suggestion<InfoTip text={"Target: "+stg.domesticDoc+"d domestic / "+stg.intlDoc+"d international"}/></th>
        <th className="py-3 px-2 w-14"/></tr>
      <tr className="bg-gray-900/60 text-xs text-gray-500 uppercase">
        <th colSpan={9}/><th className="border-l-2 border-gray-600"/>
        <th className="py-1 px-2 text-right">Qty</th><th className="py-1 px-2 text-right">Need$</th><th className="py-1 px-2 text-right">After DOC</th><th/></tr>
      </thead><tbody>{enriched.map(c=><tr key={c.id} className="border-b border-gray-800/50 hover:bg-gray-800/30 text-sm">
        <td className="py-2 px-2"><Dot status={c.status}/></td>
        <td className="py-2 px-2 text-blue-400 font-mono text-xs">{c.id}</td>
        <td className="py-2 px-2 text-gray-400 text-xs truncate max-w-[110px]" title={c.vendorCountry?c.ven+" ("+c.vendorCountry+")":c.ven}>{c.ven}</td>
        <td className="py-2 px-2 text-gray-200 truncate max-w-[200px]">{c.ti}</td>
        <td className="py-2 px-2 text-right">{R(c.dsr)}</td><td className="py-2 px-2 text-right">{R(c.d7)}</td>
        ...
        <td className={`py-2 px-2 text-right font-semibold ${dc(c.doc,c.critDays,c.warnDays)}`}>{R(c.doc)}</td>
        <td className="py-2 px-2 text-right">{R(c.allIn)}</td>
        <td className="py-2 px-1 border-l-2 border-gray-600"/>
        <td className="py-2 px-2 text-right">{c.needQty>0?R(c.needQty):"\u2014"}</td>
        <td className="py-2 px-2 text-right text-amber-300 font-semibold">{c.needDollar>0?D(c.needDollar):"\u2014"}</td>
        <td className={`py-2 px-2 text-right ${c.needQty>0?dc(c.docAfter,c.critDays,c.warnDays):"text-gray-500"}`}>{c.needQty>0?R(c.docAfter):"\u2014"}</td>
    ...
    {vm==="vendor"&&vGroups.map(grp=>{const v=grp.v;const noNeed=grp.q===0;const meets=grp.d>=(v.moqDollar||0);const td=getTargetDoc(v,stg);
      return<div key={v.name} className="mb-5 border border-gray-800 rounded-xl overflow-hidden">
        <div className="bg-gray-900 px-4 py-3 flex flex-wrap items-center gap-3">
          ...
          <span className="text-xs text-gray-400">LT:{v.lt}d</span><span className="text-xs text-gray-400">MOQ:{D(v.moqDollar)}</span>
          <span className="text-xs text-gray-400">Target:{td}d</span>
          {noNeed?<span className="ml-auto text-xs font-semibold px-2 py-0.5 rounded text-gray-400 bg-gray-700">No Need to Buy</span>
            :<span className={`ml-auto text-xs font-semibold px-2 py-0.5 rounded ${meets?"text-emerald-400 bg-emerald-400/10":"text-red-400 bg-red-400/10"}`}>{meets?"\u2713 MOQ":"\u2717 "+D(grp.d)+"/"+D(v.moqDollar)}</span>}</div>
        <table className="w-full"><thead><tr className="text-xs text-gray-500 uppercase bg-gray-900/40">
          ...
          <th className="py-2 px-2 text-right">After DOC</th><th className="py-2 px-2 w-14"/></tr></thead>
        <tbody>{grp.cores.map(c=><tr key={c.id} className="border-t border-gray-800/30 hover:bg-gray-800/20 text-sm">
          <td className="py-2 px-2"><Dot status={c.status}/></td>
          <td className="py-2 px-2 text-blue-400 font-mono text-xs">{c.id}</td>
          <td className="py-2 px-2 text-gray-200 truncate max-w-[200px]">{c.ti}</td>
          <td className="py-2 px-2 text-right">{R(c.dsr)}</td>
          <td className={`py-2 px-2 text-right font-semibold ${dc(c.doc,c.critDays,c.warnDays)}`}>{R(c.doc)}</td>
          <td className="py-2 px-2 text-right">{R(c.allIn)}</td>
          <td className="py-2 px-1 border-l-2 border-gray-600"/>
          <td className="py-2 px-2 text-right">{c.needQty>0?R(c.needQty):"\u2014"}</td>
          <td className="py-2 px-2 text-right text-amber-300">{c.needDollar>0?D(c.needDollar):"\u2014"}</td>
          <td className={`py-2 px-2 text-right ${c.needQty>0?dc(c.docAfter,c.critDays,c.warnDays):"text-gray-500"}`}>{c.needQty>0?R(c.docAfter):"\u2014"}</td>
          <td className="py-2 px-2"><button onClick={()=>onViewCore(c.id)} className="text-blue-400 text-xs px-2 py-1 bg-blue-400/10 rounded">View</button></td>
        </tr>)}{!noNeed&&<tr className="bg-gray-900/60 font-semibold text-sm"><td colSpan={6} className="py-2 px-2 text-right text-gray-300">Total:</td><td className="border-l-2 border-gray-600"/><td className="py-2 px-2 text-right">{R(grp.q)}</td><td className="py-2 px-2 text-right text-amber-300">{D(grp.d)}</td><td colSpan={2}/></tr>}</tbody></table></div>})}
  </div>}
...
function CoreDetailTab({data,stg,hist,coreId,onBack,onGoBundle}){
  ...
  const lt=vendor?.lt||30;const td=getTargetDoc(vendor,stg);
  ...
  const yTotals=useMemo(()=>{const t={};years.forEach(y=>{const r=coreHist.filter(h=>h.y===y);t[y]=r.length?+(r.reduce((s,x)=>s+x.avgDsr,0)/r.length).toFixed(1):0});return t},[coreHist,years]);
  const dsrChart=useMemo(()=>Array.from({length:12},(_,i)=>{const row={month:["Jan","Feb","Mar","Apr","May","Jun","Jul","Aug","Sep","Oct","Nov","Dec"][i]};years.forEach(y=>{const h=coreHist.find(x=>x.y===y&&x.m===i+1);row["dsr_"+y]=h?.avgDsr??null;if(h?.oosDays>0)row["oos_"+y]=h.oosDays});return row}),[coreHist,years]);
  // FIX: match bundles by core1 field (reliable) + jlsList fallback
  const coreBundlesAll=useMemo(()=>{
    if(!core)return[];
    const byCore1=(data.bundles||[]).filter(b=>b.core1===sel);
    if(byCore1.length>0)return byCore1;
    const jls=(core.jlsList||"").split(/[,\n]/).filter(Boolean).map(j=>j.trim());
    return(data.bundles||[]).filter(b=>jls.includes(b.j));
  },[core,sel,data.bundles]);
  // Units from bundleSales
  ...
  const unitsChart=useMemo(()=>{
    const byMY={};bSalesH.forEach(h=>{const k=h.y+"-"+h.m;if(!byMY[k])byMY[k]={y:h.y,m:h.m,u:0};byMY[k].u+=h.units});
    return Array.from({length:12},(_,i)=>{const row={month:["Jan","Feb","Mar","Apr","May","Jun","Jul","Aug","Sep","Oct","Nov","Dec"][i]};years.forEach(y=>{const r=byMY[y+"-"+(i+1)];row["u_"+y]=r?.u??null});return row});
  },[bSalesH,years]);
  ...
  const nq=core?calcNeedQty(core,td):0;const da=core?calcDocAfter(core,nq):0;
  ...
  const coreBundles=useMemo(()=>{
    return coreBundlesAll.map(b=>({...b,fee:feesMap[b.j],sale:salesMap[b.j],pct:core&&core.dsr>0?+((b.cd/core.dsr)*100).toFixed(1):0}))
      .sort((a,b)=>{if(bSort==="dsr")return(b.cd||0)-(a.cd||0);if(bSort==="ltP")return((b.sale?.ltP||0)-(a.sale?.ltP||0));if(bSort==="ltR")return((b.sale?.ltR||0)-(a.sale?.ltR||0));if(bSort==="lyP")return((b.sale?.lyP||0)-(a.sale?.lyP||0));if(bSort==="tyR")return((b.sale?.tyR||0)-(a.sale?.tyR||0));return 0});
  },[coreBundlesAll,feesMap,salesMap,bSort,core]);

  if(!core)return<div className="p-4 max-w-4xl mx-auto"><div className="flex items-center gap-3 mb-4"><button onClick={onBack} className="text-gray-400 hover:text-white text-sm">{"\u2190"} Purchasing</button><input type="text" placeholder="Search core ID or title..." value={search} onChange={e=>setSearch(e.target.value)} className="bg-gray-800 border border-gray-700 text-white rounded-lg px-4 py-2.5 flex-1 max-w-md text-sm"/></div>{search.length>=2?<div className="space-y-1">{(data.cores||[]).filter(c=>{const q=search.toLowerCase();return c.id.toLowerCase().includes(q)||c.ti.toLowerCase().includes(q)}).slice(0,12).map(c=><button key={c.id} onClick={()=>setSel(c.id)} className="w-full text-left px-4 py-2.5 rounded-lg bg-gray-900/50 hover:bg-gray-800 flex items-center gap-3"><Dot status={getStatus(c.doc,(data.vendors||[]).find(v=>v.name===c.ven)?.lt||30,c.buf,stg)}/><span className="text-blue-400 font-mono text-sm">{c.id}</span><span className="text-gray-300 text-sm truncate">{c.ti}</span><span className="ml-auto text-gray-500 text-xs">DSR:{R(c.dsr)} DOC:{R(c.doc)}</span></button>)}</div>:<p className="text-gray-500 text-sm">Type at least 2 characters to search.</p>}</div>;
  ...
  return<div className="p-4 max-w-6xl mx-auto">
    ...
    <div className="grid grid-cols-2 sm:grid-cols-5 gap-3 mb-4">
      {[{l:"C.DSR",v:R(core.dsr)},{l:"7D DSR",v:R(core.d7)},{l:"DOC",v:R(core.doc),c:core.doc<=lt?"text-red-400":core.doc<=lt+(core.buf||14)?"text-amber-400":"text-emerald-400"},{l:"All-In Own Pcs",v:R(allIn)},{l:"Inbound",v:R(core.inb)}].map(k=><div key={k.l} className="bg-gray-900 rounded-lg p-3 border border-gray-800"><div className="text-gray-500 text-xs mb-1">{k.l}</div><div className={`text-lg font-bold ${k.c||"text-white"}`}>{k.v}</div></div>)}</div>
    {coreHist.length>0&&<div className="grid grid-cols-1 lg:grid-cols-2 gap-4 mb-4">
      <div className="bg-gray-900 rounded-xl p-4 border border-gray-800"><h3 className="text-white font-semibold text-sm mb-1">Monthly DSR (YoY)</h3>{oosYears.length>0&&<p className="text-red-400 text-xs mb-2 font-semibold">OOS: {oosYears.join(", ")}</p>}<ResponsiveContainer width="100%" height={260}><ComposedChart data={dsrChart}><CartesianGrid strokeDasharray="3 3" stroke="#374151"/><XAxis dataKey="month" tick={{fill:"#9ca3af",fontSize:11}}/><YAxis yAxisId="left" tick={{fill:"#9ca3af",fontSize:11}}/>{oosYears.length>0&&<YAxis yAxisId="right" orientation="right" tick={{fill:"#ef4444",fontSize:11}}/>}<Tooltip {...TT} formatter={(v,n)=>{if(v==null)return[null,null];if(n.startsWith("oos_"))return v?[v+"d","OOS "+n.split("_")[1]]:[null,null];return[Math.round(v),"DSR "+n.split("_")[1]]}}/><Legend formatter={v=>v.startsWith("oos_")?"OOS "+v.split("_")[1]:"DSR "+v.split("_")[1]+" ("+((yTotals||{})[v.split("_")[1]]||"\u2014")+")"}/>{years.map(y=><Bar key={"dsr_"+y} yAxisId="left" dataKey={"dsr_"+y} fill={YR_C[y]||"#6b7280"} opacity={0.85} radius={[2,2,0,0]}/>)}{oosYears.map(y=><Bar key={"oos_"+y} yAxisId="right" dataKey={"oos_"+y} fill={OOS_C[y]} opacity={0.8} barSize={6} radius={[2,2,0,0]}/>)}</ComposedChart></ResponsiveContainer></div>
      {bSalesH.length>0&&<div className="bg-gray-900 rounded-xl p-4 border border-gray-800"><h3 className="text-white font-semibold text-sm mb-3">Monthly Sales Units (YoY)</h3><ResponsiveContainer width="100%" height={260}><LineChart data={unitsChart}><CartesianGrid strokeDasharray="3 3" stroke="#374151"/><XAxis dataKey="month" tick={{fill:"#9ca3af",fontSize:11}}/><YAxis tick={{fill:"#9ca3af",fontSize:11}}/><Tooltip {...TT}/><Legend/>{years.map(y=><Line key={"u_"+y} dataKey={"u_"+y} stroke={YR_C[y]||"#6b7280"} strokeWidth={2} dot={{r:3}} connectNulls name={""+y}/>)}</LineChart></ResponsiveContainer></div>}
    </div>}
...
function BundleDetailTab({data,stg,hist,bundleId,onBack,onGoCore}){
  ...
  const salesHist=useMemo(()=>(hist?.bundleSales||[]).filter(h=>h.j===sel).sort((a,b)=>a.month.localeCompare(b.month)),[hist,sel]);
  const priceHist=useMemo(()=>(hist?.priceHist||[]).filter(h=>h.j===sel).sort((a,b)=>a.month.localeCompare(b.month)),[hist,sel]);
  const yoyYears=useMemo(()=>[...new Set(salesHist.map(h=>h.y))].sort(),[salesHist]);
  const revData=useMemo(()=>salesHist.map(h=>({month:h.month.slice(2),rev:h.rev,profit:h.profit})),[salesHist]);
  const priceData=useMemo(()=>{const pm={};priceHist.forEach(p=>pm[p.month]=p.avgPrice);return salesHist.map(h=>({month:h.month.slice(2),units:h.units,price:pm[h.month]??null}))},[salesHist,priceHist]);
  const yoyData=useMemo(()=>Array.from({length:12},(_,i)=>{const r={month:["Jan","Feb","Mar","Apr","May","Jun","Jul","Aug","Sep","Oct","Nov","Dec"][i]};yoyYears.forEach(y=>{const rec=salesHist.find(h=>h.y===y&&h.m===i+1);r["u_"+y]=rec?.units??null});return r}),[salesHist,yoyYears]);
  ...
  return<div className="p-4 max-w-6xl mx-auto">
    ...
    {salesHist.length>0&&<div className="grid grid-cols-1 lg:grid-cols-2 gap-4 mb-4">
      <div className="bg-gray-900 rounded-xl p-4 border border-gray-800"><h3 className="text-white font-semibold text-sm mb-3">Sales History</h3><ResponsiveContainer width="100%" height={240}><BarChart data={salesHist.map(h=>({month:h.month.slice(2),units:h.units}))}><CartesianGrid strokeDasharray="3 3" stroke="#374151"/><XAxis dataKey="month" tick={{fill:"#9ca3af",fontSize:9}} angle={-45} textAnchor="end" height={45}/><YAxis tick={{fill:"#9ca3af",fontSize:11}}/><Tooltip {...TT}/><Bar dataKey="units" fill={BLUE} radius={[2,2,0,0]}/></BarChart></ResponsiveContainer></div>
      <div className="bg-gray-900 rounded-xl p-4 border border-gray-800"><h3 className="text-white font-semibold text-sm mb-3">Price History</h3><ResponsiveContainer width="100%" height={240}><ComposedChart data={priceData}><CartesianGrid strokeDasharray="3 3" stroke="#374151"/><XAxis dataKey="month" tick={{fill:"#9ca3af",fontSize:9}} angle={-45} textAnchor="end" height={45}/><YAxis yAxisId="left" tick={{fill:"#9ca3af",fontSize:11}}/><YAxis yAxisId="right" orientation="right" tick={{fill:"#eab308",fontSize:11}}/><Tooltip {...TT}/><Legend/><Bar yAxisId="left" dataKey="units" fill={BLUE} opacity={0.3} radius={[2,2,0,0]}/><Line yAxisId="right" dataKey="price" stroke={YELLOW} strokeWidth={2} type="stepAfter" dot={false}/></ComposedChart></ResponsiveContainer></div>
      <div className="bg-gray-900 rounded-xl p-4 border border-gray-800"><h3 className="text-white font-semibold text-sm mb-3">Revenue & Profit</h3><ResponsiveContainer width="100%" height={240}><BarChart data={revData}><CartesianGrid strokeDasharray="3 3" stroke="#374151"/><XAxis dataKey="month" tick={{fill:"#9ca3af",fontSize:9}} angle={-45} textAnchor="end" height={45}/><YAxis tick={{fill:"#9ca3af",fontSize:11}}/><Tooltip {...TT}/><Legend/><Bar dataKey="rev" fill={BLUE} name="Revenue" radius={[2,2,0,0]}/><Bar dataKey="profit" fill={GREEN} name="Profit" radius={[2,2,0,0]}/></BarChart></ResponsiveContainer></div>
...
export default function App(){
  ...
  const load=useCallback(()=>{setLoading(true);setError(null);apiCall('live').then(d=>{setData({cores:d.cores||[],bundles:d.bundles||[],vendors:d.vendors||[],sales:d.sales||[],fees:d.fees||[]});setTs(d.timestamp||"");setLoading(false);apiCall('history').then(h=>{setHist(h);setHistReady(true)}).catch(()=>{})}).catch(e=>{setError(e.message);setLoading(false)})},[]);
  useEffect(()=>{load()},[load]);
  ...
  return<div className="min-h-screen bg-gray-950 text-gray-200">
    <header className="bg-gray-900 border-b border-gray-800 px-4 py-3 sticky top-0 z-40"><div className="flex items-center justify-between max-w-7xl mx-auto"><div className="flex items-center gap-3"><h1 className="text-white font-bold text-lg">FBA Dashboard</h1><span className="text-xs text-emerald-400 bg-emerald-400/10 px-2 py-0.5 rounded font-medium">LIVE {"\u2014"} {data.cores.length} cores</span>{fmtTs(ts)&&<span className="text-xs text-gray-500">{fmtTs(ts)}</span>}{!histReady&&<span className="text-xs text-yellow-500 animate-pulse">Loading charts...</span>}</div><div className="flex items-center gap-3"><div className="flex gap-2 text-xs"><span className="flex items-center gap-1"><span className="w-2 h-2 rounded-full bg-red-500"/>{sc.critical}</span><span className="flex items-center gap-1"><span className="w-2 h-2 rounded-full bg-amber-500"/>{sc.warning}</span><span className="flex items-center gap-1"><span className="w-2 h-2 rounded-full bg-emerald-500"/>{sc.healthy}</span></div><button onClick={load} className="text-gray-400 hover:text-white text-sm px-2 py-1 rounded hover:bg-gray-800">{"\u21BB"}</button><button onClick={()=>setShowS(true)} className="text-gray-400 hover:text-white text-lg px-2 py-1 rounded hover:bg-gray-800">{"\u2699\uFE0F"}</button></div></div></header>
    ...
    <main className="max-w-7xl mx-auto">
      ...
      {tab==="purchasing"&&<PurchasingTab data={data} stg={stg} onViewCore={goCore}/>}
