...
function calcOrderQty(needQty,moq){return needQty<=0?0:Math.max(needQty,moq||0)}
function calcDocAfter(c,orderQty){return orderQty<=0?Math.round(c.doc):c.dsr>0?Math.round((calcAllIn(c)+orderQty)/c.dsr):999}
...
function fmtEta(s){if(!s)return"";try{const p=s.split("-");if(p.length===3)return MN[parseInt(p[1])-1]+" "+parseInt(p[2])+", "+p[0];return s}catch(e){return s}}
// Chart label: "2024-06" -> "Jun 24"
function chartLbl(month){if(!month||month.length<7)return month;const p=month.split("-");const mi=parseInt(p[1])-1;return(MN[mi]||"")+" "+p[0].slice(2)}
...
const DEF_GL=[{term:"C.DSR",desc:"Composite Daily Sales Rate."},{term:"DOC",desc:"Days of Coverage = All-In / DSR."},{term:"Qty Needed",desc:"Units to reach Target DOC."},{term:"Order Qty",desc:"max(Qty Needed, MOQ). Minimum you can actually order."},{term:"Need$",desc:"Order Qty x Unit Cost."},{term:"After DOC",desc:"(All-In + Order Qty) / DSR."},{term:"Seasonal",desc:"CV of monthly DSR > 0.3. Shows peak quarter."}];
...
function PurchasingTab({data,stg,onViewCore}){
  ...
  const enriched=useMemo(()=>{
    return(data.cores||[]).filter(c=>{
      ...
      const allIn=calcAllIn(c);const nq=calcNeedQty(c,td);const oq=calcOrderQty(nq,c.moq);
  ...
  return<div className="p-4">
    ...
    {vm==="core"&&<div className="overflow-x-auto rounded-xl border border-gray-800"><table className="w-full"><thead>
      ...
      <td className="py-2 px-2 text-right text-gray-300">{c.needQty>0?R(c.needQty):"\u2014"}</td>
      <td className="py-2 px-2 text-right text-white font-semibold">{c.orderQty>0?R(c.orderQty):"\u2014"}</td>
    ...
    {vm==="vendor"&&vGroups.map(grp=>{const v=grp.v;const noNeed=grp.nq===0;const meets=grp.d>=(v.moqDollar||0);const td=getTD(v,stg);
      return<div key={v.name} className="mb-5 border border-gray-800 rounded-xl overflow-hidden">
        ...
        <tbody>{grp.cores.map(c=><tr key={c.id} className="border-t border-gray-800/30 hover:bg-gray-800/20 text-sm">
          <td className="py-2 px-2"><Dot status={c.status}/></td><td className="py-2 px-2 text-blue-400 font-mono text-xs">{c.id}</td><td className="py-2 px-2 text-gray-200 truncate max-w-[180px]">{c.ti}</td><td className="py-2 px-2 text-right">{R(c.dsr)}</td><td className={`py-2 px-2 text-right font-semibold ${dc(c.doc,c.critDays,c.warnDays)}`}>{R(c.doc)}</td><td className="py-2 px-2 text-right">{R(c.allIn)}</td><td className="py-2 px-2 text-right text-gray-400 text-xs">{c.moq>0?R(c.moq):"\u2014"}</td><td className="py-2 px-1 border-l-2 border-gray-600"/>
          <td className="py-2 px-2 text-right text-gray-300">{c.needQty>0?R(c.needQty):"\u2014"}</td><td className="py-2 px-2 text-right text-white font-semibold">{c.orderQty>0?R(c.orderQty):"\u2014"}</td><td className="py-2 px-2 text-right text-amber-300">{c.needDollar>0?D(c.needDollar):"\u2014"}</td><td className={`py-2 px-2 text-right ${c.orderQty>0?dc(c.docAfter,c.critDays,c.warnDays):"text-gray-500"}`}>{c.orderQty>0?R(c.docAfter):"\u2014"}</td><td className="py-2 px-2"><button onClick={()=>onViewCore(c.id)} className="text-blue-400 text-xs px-2 py-1 bg-blue-400/10 rounded">View</button></td>
        </tr>)}{!noNeed&&<tr className="bg-gray-900/60 font-semibold text-sm"><td colSpan={7} className="py-2 px-2 text-right text-gray-300">Total:</td><td className="border-l-2 border-gray-600"/><td className="py-2 px-2 text-right text-gray-300">{R(grp.nq)}</td><td className="py-2 px-2 text-right">{R(grp.q)}</td><td className="py-2 px-2 text-right text-amber-300">{D(grp.d)}</td><td colSpan={2}/></tr>}</tbody></table></div>})}
  </div>}
...
function CoreDetailTab({data,stg,hist,coreId,onBack,onGoBundle}){
  ...
  return<div className="p-4 max-w-6xl mx-auto">
    ...
    <div className="bg-gray-900 rounded-xl p-4 mb-4 border border-gray-800">
      ...
      <p className="text-gray-500 text-xs">{core.ven} {"\u00B7"} {D2(core.cost)} {"\u00B7"} LT:{lt}d {"\u00B7"} Target:{td}d {"\u00B7"} {core.cat}{inboundShipments.length>0&&<span className="text-emerald-400 ml-2">{"\u{1F4E6}"} {inboundShipments.length} inbound{inboundShipments.map((s,i)=>s.eta?<span key={i} className="ml-1">ETA:{fmtEta(s.eta)}</span>:null)}</span>}</p></div>
    <div className="grid grid-cols-2 sm:grid-cols-5 gap-3 mb-4">
      {[{l:"C.DSR",v:R(core.dsr)},{l:"7D DSR",v:R(core.d7)},{l:"DOC",v:R(core.doc),c:core.doc<=lt?"text-red-400":core.doc<=lt+(core.buf||14)?"text-amber-400":"text-emerald-400"},{l:"All-In Own Pcs",v:R(allIn)},{l:"Inbound",v:R(core.inb),sub:inboundShipments.length>0?inboundShipments.map(s=>s.eta?"ETA: "+fmtEta(s.eta):"").filter(Boolean).join(", "):null}].map(k=><div key={k.l} className="bg-gray-900 rounded-lg p-3 border border-gray-800"><div className="text-gray-500 text-xs mb-1">{k.l}</div><div className={`text-lg font-bold ${k.c||"text-white"}`}>{k.v}</div>{k.sub&&<div className="text-emerald-400 text-xs mt-1">{k.sub}</div>}</div>)}</div>
...
function BundleDetailTab({data,stg,hist,bundleId,onBack,onGoCore}){
  ...
  const revData=useMemo(()=>salesHist.map(h=>({month:chartLbl(h.month),rev:h.rev,profit:h.profit})),[salesHist]);
  const priceData=useMemo(()=>{const pm={};priceHist.forEach(p=>pm[p.month]=p.avgPrice);return salesHist.map(h=>({month:chartLbl(h.month),units:h.units,price:pm[h.month]??null}))},[salesHist,priceHist]);
  ...
  return<div className="p-4 max-w-6xl mx-auto">
    ...
    {salesHist.length>0&&<div className="grid grid-cols-1 lg:grid-cols-2 gap-4 mb-4">
      <div className="bg-gray-900 rounded-xl p-4 border border-gray-800"><h3 className="text-white font-semibold text-sm mb-3">Sales History</h3><ResponsiveContainer width="100%" height={240}><BarChart data={salesHist.map(h=>({month:chartLbl(h.month),units:h.units}))}><CartesianGrid strokeDasharray="3 3" stroke="#374151"/><XAxis dataKey="month" tick={{fill:"#9ca3af",fontSize:9}} angle={-45} textAnchor="end" height={50}/><YAxis tick={{fill:"#9ca3af",fontSize:11}}/><Tooltip {...TT}/><Bar dataKey="units" fill={BLUE} radius={[2,2,0,0]}/></BarChart></ResponsiveContainer></div>
